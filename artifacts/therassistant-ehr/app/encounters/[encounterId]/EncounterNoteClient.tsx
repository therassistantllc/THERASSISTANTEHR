...
type EncounterCodingReportDocument = {
  id: string;
  type: string | null;
  title: string | null;
...
function formatDocumentTypeLabel(value: string | null | undefined): string {
  ...
  if (normalized === "coding_report") return "Coding report";
  return normalized
    .split("_")
    .filter(Boolean)
...
export default function EncounterNoteClient({ encounterId }: { encounterId: string }) {
  ...
  const [codingReportDocs, setCodingReportDocs] = useState<EncounterCodingReportDocument[]>([]);
  ...
  const personalTemplates = useMemo(() => templates.filter((t) => t.provider_id !== null), [templates]);
  const orgTemplates = useMemo(() => templates.filter((t) => t.provider_id === null), [templates]);
  ...
  }, [soapNote, summary?.encounter?.service_date, summary?.encounter?.started_at, summary?.encounter?.ended_at, summary?.appointment?.service_location, summary?.appointment?.telehealth_url]);
  ...
  const medicaidCodeTags = useMemo(() => {
    ...
    return medicaidSuggestions.recommendations
      ...
      .map((rec) => ({ code: rec.code, className: "status status-green" }));
  }, [medicaidSuggestions]);
  ...
  async function loadEncounter() {
    ...
    try {
      ...
      if (appointmentIdForCheckIn && !mergedSubjective.includes(CHECK_IN_SUBJECTIVE_MARKER)) {
        try {
          ...
          if (checkInJson?.success && ci && ci.status === "submitted") {
            const block = composeCheckInSubjectiveBlock({ focusOption: ci.focusOption, focusReflection: ci.focusReflection });
            if (block) {
      ...
      setDiagnoses((json.diagnoses ?? []).map((d) => ({
  ...
  useEffect(() => {
    let cancelled = false;
    async function loadCodingReport() {
      if (!organizationId) return;
      try {
        const response = await fetch(`/api/encounters/${encounterId}/documents?organizationId=${encodeURIComponent(organizationId)}`, { cache: "no-store" });
        const json = (await response.json()) as { success?: boolean; documents?: EncounterCodingReportDocument[] };
        if (cancelled) return;
        const reports = Array.isArray(json.documents) ? json.documents.filter((doc) => doc.type === "coding_report") : [];
        setCodingReportDocs(reports.slice(0, 1));
      } catch {
        /* coding report is best-effort */
      }
    }
    loadCodingReport();
  ...
  const codingReport = codingReportDocs[0] ?? null;
  ...
  useEffect(() => {
    ...
    async function loadTemplates() {
      if (!organizationId) return;
      try {
        const response = await fetch(`/api/note-templates?organizationId=${encodeURIComponent(organizationId)}`, { cache: "no-store" });
  ...
  function applyTemplate(templateId: string) {
    ...
    setSoapNote(next);
    if (skipped.length === 0 && filled.length > 0) setMessage(`Applied template "${template.name}".`);
    else if (skipped.length > 0 && filled.length > 0) setMessage(`Applied template "${template.name}" to empty sections. Kept your existing ${skipped.join(", ")} content.`);
    else if (skipped.length > 0 && filled.length === 0) setMessage(`Template "${template.name}" not applied — every section already has content.`);
  }

  async function saveAsPersonalTemplate() {
    if (typeof window === "undefined") return;
    const hasContent = (soapNote.subjective ?? "").trim().length > 0 || (soapNote.objective ?? "").trim().length > 0 || (soapNote.plan ?? "").trim().length > 0;
    if (!hasContent) {
      setError("Add some content to the Subjective, Interventions, or Plan section before saving as a template.");
      return;
    }
    const defaultName = summary?.appointment?.appointment_type ? `My ${summary.appointment.appointment_type} template` : "My personal template";
    ...
    try {
      ...
      if (!response.ok || !json.success || !json.template) throw new Error(json.error ?? "Failed to save personal template");
  ...
  async function persistNote(action: "save" | "amend" | "sign", codingReport?: CodingHelperReport) {
    const response = await fetch(`/api/encounters/${encounterId}/note`, {
      ...
      body: JSON.stringify({
        ...
        plan: soapNote.plan || "",
        userId: action === "save" ? undefined : null,
        codingReport,
      }),
    });
    const json = (await response.json()) as { success?: boolean; error?: string };
  ...
  async function saveDraftAndBillingDetails() {
    setSaving(true);
    setError(null);
    setMessage(null);
    try {
      ...
      await persistNote("save");
      setMessage("Draft and billing details saved.");
      await loadEncounter();
    } catch (saveError) {
  ...
  async function saveCodingHelperReportToChart(reportToSave: CodingHelperReport) {
    ...
    try {
      const response = await fetch(`/api/encounters/${encounterId}/coding-report`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ organizationId, report: reportToSave }),
      });
      const json = (await response.json().catch(() => ({}))) as { success?: boolean; error?: string; title?: string };
      if (!response.ok || !json.success) throw new Error(json.error ?? "Failed to save coding report to chart");
      setMessage(`Saved coding report to chart${json.title ? `: ${json.title}` : "."}`);
      const refreshed = await fetch(`/api/encounters/${encounterId}/documents?organizationId=${encodeURIComponent(organizationId)}`, { cache: "no-store" });
      const refreshedJson = (await refreshed.json().catch(() => ({}))) as { success?: boolean; documents?: EncounterCodingReportDocument[] };
      if (refreshedJson.success && Array.isArray(refreshedJson.documents)) setCodingReportDocs(refreshedJson.documents.filter((doc) => doc.type === "coding_report").slice(0, 1));
    } catch (saveError) {
  ...
  async function handleJournalImport({ entry, field, text }: ImportResult) {
    ...
    if (!noteId) {
      const res = await fetch(`/api/encounters/${encounterId}/summary?organizationId=${encodeURIComponent(organizationId)}`, { cache: "no-store" });
      const json = (await res.json()) as EncounterSummary;
      noteId = json.clinicalNote?.id ?? null;
    }
    if (!noteId) throw new Error("Could not create a draft note to import into. Try Save Draft first.");
    const clientIdForEntry = summary?.client?.id ?? "";
    const importRes = await fetch(`/api/clients/${encodeURIComponent(clientIdForEntry)}/journal/${entry.id}/import`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (!importRes.ok || !importJson.success) throw new Error(importJson.error ?? "Failed to mark entry as imported");
    ...
    const saveRes = await fetch(`/api/encounters/${encounterId}/note`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ organizationId, action: isSigned ? "amend" : "save", subjective: updated.subjective ?? "", objective: updated.objective ?? "", assessment: updated.assessment ?? "", plan: updated.plan ?? "", userId: null }),
    });
    if (!saveRes.ok) throw new Error("Imported entry was marked, but saving the appended note text failed. Use Save Draft to retry.");
  ...
  return (
    <>
        ...
        <aside className="workspace-sidebar-left">
          <article className="panel">
            ...
            <div className="detail-list">
              ...
              {summary.coverage?.isMedicaid ? <p><strong>Coverage:</strong> Medicaid{summary.coverage.primaryPayerName ? ` (${summary.coverage.primaryPayerName})` : ""}</p> : null}
        ...
        <main className="workspace-main">
          <article className="panel template-picker-panel">
            <div className="template-picker-row">
              <label htmlFor="note-template-picker"><strong>Note template</strong><span className="muted" style={{ marginLeft: "0.5rem", fontSize: "0.875rem" }}>Applies to empty sections only — anything you&apos;ve typed is kept.</span></label>
              <div className="template-picker-controls">
                <select id="note-template-picker" value={selectedTemplateId} onChange={(e) => applyTemplate(e.target.value)} disabled={finalized || saving || templates.length === 0}>
                  <option value="">{templates.length === 0 ? "No templates available" : "Choose a template…"}</option>
                  {personalTemplates.length > 0 ? <optgroup label="My personal templates">{personalTemplates.map((template) => <option key={template.id} value={template.id}>{template.name}{template.service_type ? ` · ${template.service_type}` : ""}{template.cpt_code ? ` · ${template.cpt_code}` : ""}</option>)}</optgroup> : null}
                  {orgTemplates.length > 0 ? <optgroup label="Organization templates">{orgTemplates.map((template) => <option key={template.id} value={template.id}>{template.name}{template.is_default ? " (default)" : ""}{template.service_type ? ` · ${template.service_type}` : ""}{template.cpt_code ? ` · ${template.cpt_code}` : ""}</option>)}</optgroup> : null}
                </select>
                <button type="button" className="button button-secondary" onClick={saveAsPersonalTemplate} disabled={finalized || saving || savingPersonal} title="Save the current note as a personal template only you can see.">{savingPersonal ? "Saving…" : "Save as personal template"}</button>
              </div>
            </div>
          </article>
          ...
          <article className="panel">
            <h2>Coding Report</h2>
            {!codingReport ? <p className="muted" style={{ margin: 0 }}>No coding report has been saved for this encounter yet.</p> : (
              <table className="data-table"><thead><tr><th>Report</th><th>Type</th><th>Filed</th></tr></thead><tbody><tr key={codingReport.id}><td><strong>{codingReport.title ?? codingReport.fileName ?? "Coding report"}</strong>{codingReport.title && codingReport.fileName ? <div className="muted" style={{ fontSize: 12 }}>{codingReport.fileName}</div> : null}</td><td>{formatDocumentTypeLabel(codingReport.type)}</td><td>{formatDate(codingReport.filedAt ?? codingReport.createdAt)}</td></tr></tbody></table>
            )}
          </article>

          <article className="panel">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <div><h2 style={{ margin: 0 }}>Between-session journal</h2><p className="muted" style={{ margin: "4px 0 0 0", fontSize: 13 }}>Pull individual entries the client logged between visits into a SOAP field.</p></div>
              <button type="button" className="button button-secondary" onClick={() => setShowJournalModal(true)} disabled={!summary?.client?.id}>Import from journal</button>
            </div>
          </article>
          ...
          {placeOfServiceWarningMessage ? <div style={{ padding: "10px 12px", borderRadius: 6, background: "#FEF2F2", border: "1px solid #FECACA", color: "#991B1B", fontSize: 13 }}>{placeOfServiceWarningMessage}</div> : null}
          ...
          {summary.coverage?.isMedicaid && medicaidCodeTags.length > 0 ? <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginTop: "-0.75rem" }}><span className="muted" style={{ fontSize: 12 }}>Suggested codes:</span>{medicaidCodeTags.map((tag) => <span key={`${tag.code}-${tag.className}`} className={tag.className} style={{ fontSize: 11, letterSpacing: 0.2 }}>{tag.code}</span>)}</div> : null}
        </main>
      </section>
      ...
      {showCodingHelper ? (
        <div role="dialog" aria-modal="true" aria-label="Coding helper" onClick={(e) => { if (e.target === e.currentTarget) setShowCodingHelper(false); }} style={{ position: "fixed", inset: 0, background: "rgba(15, 23, 42, 0.45)", display: "flex", justifyContent: "center", alignItems: "flex-start", padding: "48px 16px", zIndex: 1000, overflowY: "auto" }}>
          <div className="panel" style={{ background: "var(--surface-color, #fff)", borderRadius: 8, width: "min(980px, 100%)", maxHeight: "calc(100vh - 96px)", overflowY: "auto", padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}><div><h2 style={{ margin: 0 }}>Coding Helper</h2><p className="muted" style={{ margin: "4px 0 0 0", fontSize: 13 }}>Native encounter coding support that uses live SOAP, diagnosis, and service line state.</p></div><button className="button button-secondary" type="button" onClick={() => setShowCodingHelper(false)}>Close</button></div>
            <div style={{ marginTop: 12, overflow: "auto", maxHeight: "72vh" }}>
              <CodingHelperPanel encounterId={encounterId} organizationId={organizationId} practiceName={summary.practice?.legalName ?? summary.practice?.name ?? undefined} providerName={summary.provider?.name ?? undefined} dateOfService={summary.encounter?.service_date} clientName={client?.name} payerName={summary.coverage?.primaryPayerName} isMedicaid={!!summary.coverage?.isMedicaid} soapNote={soapNote} diagnoses={diagnoses} serviceLines={serviceLines} onApplySuggestedCodes={(codes) => { setServiceLines((prev) => prev.length ? prev.map((line, i) => i === 0 ? { ...line, cpt_hcpcs_code: codes[0] ?? line.cpt_hcpcs_code } : line) : prev); }} onSaveReport={async (report) => { setGeneratedCodingReport(report); await saveCodingHelperReportToChart(report); }} />
            </div>
          </div>
        </div>
      ) : null}
      ...
      {showJournalModal && summary?.client?.id ? (
        <div role="dialog" aria-modal="true" aria-label="Import from journal" onClick={(e) => { if (e.target === e.currentTarget) setShowJournalModal(false); }} style={{ position: "fixed", inset: 0, background: "rgba(15, 23, 42, 0.45)", display: "flex", justifyContent: "center", alignItems: "flex-start", padding: "48px 16px", zIndex: 1000, overflowY: "auto" }}>
          <div className="panel" style={{ background: "var(--surface-color, #fff)", borderRadius: 8, width: "min(900px, 100%)", maxHeight: "calc(100vh - 96px)", overflowY: "auto", padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><h2 style={{ margin: 0 }}>Import from client journal</h2><button type="button" className="button button-secondary" onClick={() => setShowJournalModal(false)}>Close</button></div>
            <ClinicianJournalPanel clientId={summary.client.id} organizationId={organizationId} mode="import" windowSinceLastSigned excludeEncounterId={encounterId} onImport={handleJournalImport} />
          </div>
        </div>
      ) : null}
