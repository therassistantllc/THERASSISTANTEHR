...
export default function EncounterNoteClient({ encounterId }: { encounterId: string }) {
  ...
  const isSigned = useMemo(
    () =>
      summary?.encounter?.encounter_status === "signed" ||
      summary?.clinicalNote?.note_status === "signed",
    [summary],
  );
    ...
    async function loadCodingReport() {
      ...
      try {
        ...
        const json = (await response.json()) as {
          success?: boolean;
          documents?: EncounterCodingReportDocument[];
        };
        if (cancelled) return;

        const reports = Array.isArray(json.documents)
          ? json.documents.filter((doc) => doc.type === "coding_report")
          : [];
  ...
  function applyTemplate(templateId: string) {
    ...
    if (skipped.length === 0 && filled.length > 0) {
      ...
      setMessage(`Applied template "${template.name}" to empty sections. Kept your existing ${skipped.join(", ")} content.`);
    } else if (skipped.length > 0 && filled.length === 0) {
      setMessage(`Template "${template.name}" not applied — every section already has content.`);
    }
  }
  ...
  async function saveAsPersonalTemplate() {
    ...
    const defaultName = summary?.appointment?.appointment_type
  ...
  async function saveCodingHelperReportToChart(reportToSave: CodingHelperReport) {
    ...
    try {
      ...
      const refreshedJson = (await refreshed.json().catch(() => ({}))) as {
        success?: boolean;
        documents?: EncounterCodingReportDocument[];
      };
      if (refreshedJson.success && Array.isArray(refreshedJson.documents)) {
        setCodingReportDocs(refreshedJson.documents.filter((doc) => doc.type === "coding_report").slice(0, 1));
      }
    } catch (saveError) {
  ...
  async function handleJournalImport({ entry, field, text }: ImportResult) {
    ...
    const importJson = (await importRes.json().catch(() => ({}))) as {
      success?: boolean;
      error?: string;
    };
    ...
    if (!saveRes.ok) {
      throw new Error("Imported entry was marked, but saving the appended note text failed. Use Save Draft to retry.");
    }
    ...
    <>
      <section className="hero-panel">
        ...
        <div className="hero-actions">
          ...
          <button className="button button-secondary" type="button" onClick={() => setShowCodingHelper(true)}>
            Coding Helper
          </button>
      ...
      <section className="encounter-workspace">
        ...
        <main className="workspace-main">
          ...
          <article className="panel">
            ...
            {!codingReport ? (
              <p className="muted" style={{ margin: 0 }}>
                No coding report has been saved for this encounter yet.
              </p>
            ) : (
              <table className="data-table">
                ...
                <tbody>
                  <tr key={codingReport.id}>
                    <td>
                      <strong>{codingReport.title ?? codingReport.fileName ?? "Coding report"}</strong>
                      {codingReport.title && codingReport.fileName ? (
                        <div className="muted" style={{ fontSize: 12 }}>{codingReport.fileName}</div>
                      ) : null}
                    </td>
                    <td>{formatDocumentTypeLabel(codingReport.type)}</td>
                    <td>{formatDate(codingReport.filedAt ?? codingReport.createdAt)}</td>
                  </tr>
                </tbody>
              </table>
            )}
          ...
          <CptCodePanel
            ...
            disabled={finalized}
            serviceDate={encounter.service_date || undefined}
