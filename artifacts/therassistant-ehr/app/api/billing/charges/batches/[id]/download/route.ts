...
async function markBatchDownloaded(params: { orgId: string; batchId: string }) {
  const supabase = createServerSupabaseAdminClient();
  if (!supabase) throw new Error("Database connection not available");
  ...
  if (error) throw new Error(error.message ?? "Failed to mark batch downloaded");
}
...
export async function GET(request: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    await markBatchDownloaded({ orgId: guard.organizationId, batchId: id });
