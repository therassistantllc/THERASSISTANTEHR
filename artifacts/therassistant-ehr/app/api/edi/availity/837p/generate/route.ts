...
function normalizeConnection(row: Record<string, unknown>): AvailityConnection {
  ...
  return {
    ...
    // OA Companion Guide p. 11 specifies the literal "AVAILITY" (with space).
...
async function insertClaimStatusEvent(
  ...
  await supabase.from("claim_status_events").insert({
    ...
    raw_payload: {
      ...
      availity_file_id: availityFileId ?? null,
    },
  ...
  try {
    ...
    const generationInput: Availity837PGenerationInput = {
      ...
      payerProfile: {
        ...
        availity_payer_id: asString((payerProfileRow as Record<string, unknown>).availity_payer_id),
    ...
    if (!validation.isValid) {
      ...
      await insertClaimStatusEvent(
        ...
        "validation",
        "validation_failed",
        "Claim failed Availity 837P validation",
        {
      ...
      return NextResponse.json(
        {
          error: "Claim failed Availity 837P validation",
          errors: validation.errors,
          warnings: validation.warnings,
        },
        { status: 400 },
    ...
    const batch = generateAvaility837PBatch(generationInput);
    ...
    await insertClaimStatusEvent(
      ...
      "837p_batch",
      "batched",
      "Claim added to generated Availity 837P batch",
      {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to generate Availity 837P batch" },
      { status: 500 },
    );
  }
}
