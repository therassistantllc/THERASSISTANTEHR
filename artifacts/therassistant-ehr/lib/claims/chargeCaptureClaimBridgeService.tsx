...
async function resolvePolicySnapshot(params: {
...
}): Promise<{ client: DbRow | null; policy: DbRow | null; payer: DbRow | null; subscriber: DbRow | null; errors: ClaimReadinessError[] }> {
  const supabase = createServerSupabaseAdminClient();
  if (!supabase) {
    return { client: null, policy: null, payer: null, subscriber: null, errors: [{ field: "system", message: "Database connection not available" }] };
  }
  ...
  if (policyError || !policy) {
    errors.push({ field: "insurance_policy", message: "No active primary insurance policy found for this client" });
    return { client: (client as DbRow | null) ?? null, policy: null, payer: null, subscriber: null, errors };
  }
  ...
  if (!payer) errors.push({ field: "payer", message: "Insurance policy has no usable payer record" });
  ...
  if (!subscriber) errors.push({ field: "subscriber", message: "Insurance policy has no usable subscriber record" });
...
async function renderingProviderTaxonomy(params: { organizationId: string; appointmentId?: string | null }) {
  ...
  return profile ? nullableText(profile.taxonomy_code) ?? nullableText(profile.taxonomy) ?? nullableText(profile.provider_taxonomy_code) : null;
}
...
function snapshotPayload(params: {
  ...
  const subscriberRelationship = text(params.subscriber.relationship_to_client) || text(params.policy.subscriber_relationship) || "self";
  const subscriberIsClient = ["self", "client", "patient", "insured"].includes(subscriberRelationship.toLowerCase());
  const subscriberAddress1 = firstDbText(params.subscriber, ["address_line1", "address_line_1", "address1", "street", "subscriber_address1"]) || (subscriberIsClient ? text(params.client.address_line_1) : "");
  const subscriberCity = firstDbText(params.subscriber, ["address_city", "city", "subscriber_city"]) || (subscriberIsClient ? text(params.client.city) : "");
  const subscriberState = (firstDbText(params.subscriber, ["address_state", "state", "subscriber_state"]) || (subscriberIsClient ? text(params.client.state) : "")).toUpperCase();
  const subscriberZip = firstDbText(params.subscriber, ["address_zip", "zip", "postal_code", "subscriber_zip"]) || (subscriberIsClient ? text(params.client.postal_code) : "");
...
async function syncExistingClaimFromCharge(params: {
  ...
  const supabase = createServerSupabaseAdminClient();
  if (!supabase) {
    return { ok: false, claimId: params.claimId, errors: [{ field: "system", message: "Database connection not available" }] };
  }
  ...
  if (claimError || !claim) {
    return { ok: false, claimId: params.claimId, errors: [{ field: "claim_id", message: "Existing professional claim was not found" }] };
  }
  ...
  const policyResolution = await resolvePolicySnapshot({
    ...
    policyId: params.charge.insurance_policy_id ? String(params.charge.insurance_policy_id) : null,
  });
  if (policyResolution.errors.length || !policyResolution.client || !policyResolution.policy || !policyResolution.payer || !policyResolution.subscriber) {
  ...
  const serviceLines = serviceLinesFromCharge(params.charge, providerResolution.renderingProviderNpi);
  const placeOfService = nullableText(params.charge.place_of_service) ?? nullableText(serviceLines[0]?.placeOfService) ?? "11";
  const totalCharge = money(serviceLines.reduce((sum, line) => sum + line.chargeAmount * (line.units ?? 1), 0));
  ...
  const { error: claimUpdateError } = await supabase
    ...
    .eq("id", params.claimId);
  if (claimUpdateError) {
    return { ok: false, claimId: params.claimId, errors: [{ field: "professional_claims", message: claimUpdateError.message }] };
  }
  ...
  if (deleteLinesError) {
    return { ok: false, claimId: params.claimId, errors: [{ field: "professional_claim_service_lines", message: deleteLinesError.message }] };
  }

  const linePayload = serviceLinePayloadFromCharge({ claimId: params.claimId, serviceLines, placeOfService });
  if (linePayload.length > 0) {
    const { error: insertLinesError } = await supabase.from("professional_claim_service_lines").insert(linePayload);
    if (insertLinesError) {
      return { ok: false, claimId: params.claimId, errors: [{ field: "professional_claim_service_lines", message: insertLinesError.message }] };
    }
  }
  ...
  if (deleteSnapshotError) {
    return { ok: false, claimId: params.claimId, errors: [{ field: "claim_parties_snapshot", message: deleteSnapshotError.message }] };
  }

  const { error: insertSnapshotError } = await supabase.from("claim_parties_snapshot").insert(snapshot);
  if (insertSnapshotError) {
    return { ok: false, claimId: params.claimId, errors: [{ field: "claim_parties_snapshot", message: insertSnapshotError.message }] };
  }
...
export async function createClaimDraftFromChargeCapture(
  ...
  const { data: charge, error: chargeError } = await supabase
    .from("charge_capture_items")
    .select("id, organization_id, encounter_id, client_id, provider_id, appointment_id, insurance_policy_id, case_id, charge_status, service_date, diagnosis_codes, service_lines, place_of_service, total_charge, claim_id")
