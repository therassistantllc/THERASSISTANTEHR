// File: lib/clearinghouse/x12/availity837P.ts
// Availity Professional Claim (837P) generator.
// Built against Availity OA Companion Guide Professional (837P) Claims, 005010X222A1.

export type Availity837PConfig = {
  submitterId: string;
...
export type AvailityBillingProvider = {
  entityType: "1" | "2";
...
export type AvailitySubscriber = {
  firstName: string;
  lastName: string;
  memberId: string;
  dateOfBirth: string;
...
export type AvailityRenderingProvider = {
  entityType: "1" | "2";
...
export type Availity837PClaimInput = {
  claimId: string;
  claimNumber: string;
  ...
  billingProvider: AvailityBillingProvider;
  subscriber: AvailitySubscriber;
  payer: AvailityPayer;
  renderingProvider?: AvailityRenderingProvider | null;
  serviceLines: AvailityServiceLine[];
};

export type Availity837PBuildResult = {
  x12: string;
  fileName: string;
...
function validateConfig(config: Availity837PConfig) {
  const errors: string[] = [];
  if (!config.submitterId) errors.push("Availity submitterId is required.");
  if (!config.submitterName) errors.push("Availity submitterName is required.");
...
export function validateAvaility837P(input: Availity837PClaimInput, config: Availity837PConfig) {
  ...
  if (bp?.address1 && /P\.?\s*O\.?\s*BOX/i.test(bp.address1)) errors.push("Availity requires a physical billing provider address; PO Box is not allowed.");
  ...
  if (!input.payer?.payerId) errors.push("Availity payer ID is required.");
...
export function buildAvaility837P(input: Availity837PClaimInput, config: Availity837PConfig): Availity837PBuildResult {
  const validationErrors = validateAvaility837P(input, config);
  ...
  // Availity Batch EDI Companion Guide: ISA08 = "030240928" (Availity D&B),
  // 1000B NM103 = "Availity", NM109 = "030240928", GS03 = "030240928".
  const receiverId = config.receiverId ?? "030240928";
  const receiverName = config.receiverName ?? "Availity";
  const gsReceiverCode = config.gsReceiverCode ?? "030240928";
  ...
  // Availity assigns its own internal Batch ID on receipt. Routing is
  // determined by ISA15 (T/P) — the opposite of Office Ally, where the
  // OATEST filename keyword drove routing. WARNING: ISA15="P" in a QA
  // environment WILL forward to the payer. Enforce test mode via ISA15="T".
  void usageIndicator;
  const fileName = `${clean(config.submitterId)}_${claimKeyword}_${input.claimNumber}_${gsDate}.837`;
