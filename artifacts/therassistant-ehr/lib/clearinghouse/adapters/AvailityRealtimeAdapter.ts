...
// Availity Real Time EDI adapter for 270/271 and 276/277.
// Based on Availity Real Time EDI Companion Guide v20250529.
...
// Availity real-time transactions use SOAP CORE Phase II/IV at
// https://gateway.availity.com:2021/core (NOT Office Ally's wsd.officeally.com
// JSON-MIME shape). The endpoints and request bodies below are placeholders
// to keep callers compiling; the SOAP CORE wrapper rewrite is a separate task.
const SOAP_ENDPOINT = "https://gateway.availity.com:2021/core";
const MIME_ENDPOINT = "https://gateway.availity.com:2021/core";
...
function getConfig(): RealtimeConfig {
  const mode = (process.env.AVAILITY_REALTIME_MODE ?? "soap").toLowerCase();
  if (mode !== "soap" && mode !== "mime") {
    throw new Error("AVAILITY_REALTIME_MODE must be soap or mime.");
  }
  ...
  return {
    ...
    receiverId: process.env.AVAILITY_REALTIME_RECEIVER_ID ?? RECEIVER_ID,
    endpoint: process.env.AVAILITY_REALTIME_ENDPOINT ?? (mode === "soap" ? SOAP_ENDPOINT : MIME_ENDPOINT),
    mode,
  };
}
...
async function postSoap(config: RealtimeConfig, transaction: RealtimeTransaction, x12: string, payloadId: string) {
  ...
  if (!response.ok) throw new Error(`Availity realtime SOAP failed ${response.status}: ${raw.slice(0, 500)}`);
  return raw;
}
...
async function postMime(config: RealtimeConfig, transaction: RealtimeTransaction, x12: string, payloadId: string) {
  ...
  const raw = await response.text();
  if (!response.ok) throw new Error(`Availity realtime MIME failed ${response.status}: ${raw.slice(0, 500)}`);
  return raw;
}
