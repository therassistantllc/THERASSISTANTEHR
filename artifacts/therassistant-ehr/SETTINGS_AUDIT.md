...
`custom_note_settings`) duplicate that concept:

- Zero code references — they appear only in `schema.sql` (v0/Vercel import leftovers).
- They are keyed by `organization_name` (varchar), **inconsistent** with the rest of the schema
  which keys on `organization_id` (uuid).
- They overlap conceptually with `system_settings['billing.defaults']` and note settings.
...
object (`organization.portal_settings`) all describe portal behavior.
...
## 3. Changes made in this pass (remove hardcoding)
...
- `lib/workflow/workflowFunctions.tsx` — replaced 2 `"11"` literals with
  `DEFAULT_OFFICE_PLACE_OF_SERVICE` (per explicit instruction to override the file's
  "DO NOT modify" header). Note: other fixture values in this file (CPT `90834`, charge
  `150.00`) remain hardcoded and are out of scope for the settings audit.
