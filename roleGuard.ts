...
async function hasBillingCompanyAccess(
  ...
  const { data, error } = await supabase
    ...
    .select("id, scopes")
    .eq("billing_company_organization_id", operatingOrganizationId)
    .eq("client_organization_id", requestOrganizationId)
    .eq("access_status", "active")
    .is("archived_at", null)
    .limit(1)
    .maybeSingle();
  if (error || !data) return false;
  const scopes = Array.isArray((data as { scopes?: unknown }).scopes)
    ? ((data as { scopes: string[] }).scopes)
    : [];
...
export async function requireAuthenticatedPaymentPoster(
  ...
  // Tenant isolation: staff may post for their own organization, or for a
  // client organization explicitly delegated to their billing-company org.
  if (
    ...
    if (
      ...
      throw new PaymentPostingForbiddenError(
        "You cannot post payments for this organization.",
      );
    }
  }
