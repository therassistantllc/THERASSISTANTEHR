...
interface EraPaymentItem {
  ...
  client: { id: string | null; displayName: string } | null;
...
function hasPostingTarget(payment: EraPaymentItem) {
  return payment.claimMatchStatus === "matched" || Boolean(payment.client?.id);
}

function isClaimlessPatientLedgerPost(payment: EraPaymentItem) {
  return payment.claimMatchStatus !== "matched" && Boolean(payment.client?.id);
}

function canPostPayment(payment: EraPaymentItem) {
  return payment.postingStatus === "ready" && hasPostingTarget(payment);
}
...
function deriveChronology(payment: EraPaymentItem): ChronoEvent[] {
  ...
  } else if (hasPostingTarget(payment)) {
    events.push({
      date: null,
      label: isClaimlessPatientLedgerPost(payment)
        ? "Awaiting patient ledger posting"
        : "Awaiting posting",
      state: "current",
    });
  } else {
...
export default function PaymentsClient() {
  ...
  const filtered = useMemo(() => {
    ...
    if (tab === "matched")
      list = list.filter(
        (p) => hasPostingTarget(p) && p.postingStatus !== "posted",
      );
    if (tab === "unmatched") list = list.filter((p) => !hasPostingTarget(p));
  ...
  const counts = useMemo(() => {
    ...
    for (const p of items) {
      if (hasPostingTarget(p) && p.postingStatus !== "posted") c.matched++;
      if (!hasPostingTarget(p)) c.unmatched++;
  ...
  const kpi = useMemo(() => {
    ...
    const blocked = items.filter(
      (p) => p.postingStatus === "blocked" || !hasPostingTarget(p),
    );
    const unapplied = items.filter((p) => hasPostingTarget(p) && p.postingStatus === "ready");
          ...
          <div className="flex-1 overflow-y-auto">
            {loading && items.length === 0 ? (
              ...
              filtered.map((pmt) => {
                ...
                const isPostable = hasPostingTarget(pmt);
                return (
                  <button
                    key={pmt.id}
                    type="button"
                    ...
                    <div
                      className={`flex w-1 shrink-0 rounded-sm ${
                        pmt.postingStatus === "posted"
                          ? "bg-emerald-500"
                          : pmt.postingStatus === "blocked" || !isPostable
                            ? "bg-rose-500"
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex items-center justify-between gap-2">
                        ...
                        >
                          ...
                          ) : !isPostable || pmt.postingStatus === "blocked" ? (
            ...
            <div className="mx-auto max-w-[1000px] p-4">
                ...
                <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-4 py-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isClaimlessPatientLedgerPost(selected) ? (
                        <span className="inline-flex items-center gap-1 rounded bg-blue-50 px-1.5 py-0.5 text-[9.5px] font-semibold text-blue-700 ring-1 ring-blue-200">
                          Patient ledger
                        </span>
              ...
              {isClaimlessPatientLedgerPost(selected) ? (
                <div className="mt-4 flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
                  <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-blue-600" strokeWidth={2.2} />
                  <div className="flex-1 text-[12px] text-blue-800">
                    <div className="font-semibold">This ERA has no matching claim.</div>
                    <div className="text-blue-700/90">
                      A patient is attached, so posting will write the charge, CAS adjustments,
                      insurance payment, and patient responsibility to the patient ledger.
                    </div>
                  </div>
                </div>
              ) : selected.claimMatchStatus !== "matched" ? (
          ...
          <aside className="hidden w-[300px] shrink-0 flex-col gap-3 overflow-y-auto border-l border-slate-200 bg-white p-4 lg:flex">
            ...
            >
              {postingId === selected.id
                ...
                : selected.postingStatus === "posted"
                  ? "✓ Posted to ledger"
                  : !hasPostingTarget(selected)
                    ? "Add patient or match claim to post"
                    : isClaimlessPatientLedgerPost(selected)
