...
function normalizedCasAdjustments(
  ...
  extra: Pick<NormalizedCasAdjustment, "lineNumber" | "procedureCode" | "serviceDate"> = {},
...
function normalizedEraCasAdjustments(row: EraClaimPaymentRow): NormalizedCasAdjustment[] {
  const claimLevel = normalizedCasAdjustments(row.cas_adjustments ?? []);
  const serviceLineLevel = (row.service_lines ?? []).flatMap((line, lineIndex) => {
    const adjustments = Array.isArray(line.adjustments)
      ? line.adjustments
      : Array.isArray(line.cas_adjustments)
        ? line.cas_adjustments
        : [];
    const procedureCode = String(line.procedureCode ?? line.procedure_code ?? "").trim() || null;
    const serviceDate =
      String(line.serviceDate ?? line.service_date ?? line.serviceDateFrom ?? line.service_date_from ?? "").trim() ||
      null;
...
function postingEntryTypeForCas(adj: NormalizedCasAdjustment): PostingLedgerEffect["entryType"] {
...
async function commitEra835Posting(
  ...
  const patientResponsibility = round2(Number(row.clp05_patient_responsibility ?? 0));
  ...
  try {
    ...
    if (row.client_id && !row.professional_claim_id) {
      await createClaimlessClientLedgerEntries(supabase, input.organizationId, row, {
        totalCharge,
        insurancePayment,
      ...
      try {
        const { recalculatePatientBalance } = await import("@/lib/billing/recalculatePatientBalance");
    ...
    try {
      ...
      const allowed = Number(row.clp03_total_charge ?? 0) - contractualAdjustment;
...
async function createClaimlessClientLedgerEntries(
  ...
  const computedResidual = round2(args.totalCharge - args.insurancePayment - payerAdjustmentTotal);
  ...
  for (const adj of args.casAdjustments) {
    ...
    entries.push({
      referenceSuffix: adj.sourceSegment.toLowerCase().replace(/[^a-z0-9]+/g, "-"),
      entryType,
      description: code
        ? `${adj.descriptionPrefix} ${code} adjustment`
        : `${adj.descriptionPrefix} adjustment ${adj.index + 1}`,
      ...
      balanceEffect: isPatientResponsibility ? 0 : adj.amount < 0 ? amount : -amount,
      groupCode: adj.groupCode,
      reasonCode: adj.reasonCode,
      sourceSegment: adj.sourceSegment,
  ...
  if (args.patientResponsibility > 0) {
    entries.push({
      ...
      entryType: "patient_responsibility",
      description: "Patient responsibility retained as the remaining account balance",
      debitAmount: 0,
      creditAmount: 0,
      balanceEffect: 0,
      groupCode: "PR",
  ...
  for (const entry of entries) {
    await createClientLedgerEntry(supabase, organizationId, payment, entry, args.now);
  }
}
...
async function createPatientInvoiceIfNeeded(
  ...
  if (responsibility <= 0 || !payment.client_id || !payment.professional_claim_id) return false;
