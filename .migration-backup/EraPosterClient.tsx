...
function patientPrefill(row: ClaimPayment): {
  firstName: string;
  lastName: string;
  dateOfBirth: string;
} | null {
  const parsed = row.parsedPatient;
  if (parsed?.firstName || parsed?.lastName || parsed?.dateOfBirth) {
    return {
      firstName: parsed.firstName ?? "",
      lastName: parsed.lastName ?? "",
      dateOfBirth: parsed.dateOfBirth ? normalizeEraDate(parsed.dateOfBirth) : "",
    };
  }
  return extractPatientPrefillFromRawSegments(row.rawSegments);
}
...
function AddPatientModal({
  ...
  const prefill = patientPrefill(row);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const bindBody: Record<string, string> = { organizationId, clientId };
      if (row.professionalClaim?.id) bindBody.professionalClaimId = row.professionalClaim.id;
      ...
      if (row.professionalClaim?.id) {
        ...
        await onCreated("Client created and attached. Payment can post to the patient ledger now.");
      }
    ...
    try {
      const res = await fetch("/api/billing/payment-adjustments", {
        ...
        body: JSON.stringify({
          ...
          groupCode: adjustmentDraft.groupCode.trim() || null,
          reasonCode: adjustmentDraft.reasonCode.trim() || null,
          referenceId: adjustmentDraft.referenceId.trim() || null,
          amount: amt,
          ...
          <table className={styles.postingTable}>
            ...
            <tbody>
              {claimPayments.map((rawRow) => {
                ...
                const hasPatient = Boolean(row.client?.id);
                  ...
                  >
                    ...
                    <td>
                      <div className={styles.row}>
                        ...
                        {row.claimMatchStatus === "matched" && !row.client?.id ? (
                          <button
                            className={styles.btn}
                            onClick={(ev) => {
                              ev.stopPropagation();
                ...
                <tr>
                  ...
                  <td>
                    <div style={{ display: "flex", gap: 6 }}>
                      <input
                        className={styles.inlineInput}
                        style={{ width: 54, textAlign: "left" }}
                        placeholder="Grp"
                        value={adjustmentDraft.groupCode}
                        onChange={(e) =>
                          setAdjustmentDraft({
                            ...adjustmentDraft,
                      ...
                      <input
                        ...
                        placeholder="Reason"
                        value={adjustmentDraft.reasonCode}
                        onChange={(e) =>
                          setAdjustmentDraft({ ...adjustmentDraft, reasonCode: e.target.value })
                        }
                      />
                    </div>
                  </td>
                  ...
                  <td>
                    <input
                      ...
                      placeholder="Ref"
                      value={adjustmentDraft.referenceId}
                      onChange={(e) =>
                        setAdjustmentDraft({ ...adjustmentDraft, referenceId: e.target.value })
                      }
                    />
                  </td>
